...
SUITES=(
    ...
    test_task_echo.py
    # 2026-09-23 新增、2026-09-24 扩到 91 项：地图"定位到错误地区"守卫
    # （恩施大峡谷被静默定位到太湖那次）。
    # 它钉住这些事：
    #   · 后端回传 AOI bbox、前端不再硬编码地点兜底
    #   · 生成代码不再把非内置地名的坐标默默写作太湖
    #   · 区域库规模/坐标合法性/half 区间/无重名
    #   · **排序键与匹配键同源**（别名与正式名等价，长度排序必须取两者最大值）
    #   · 16 组长短名歧义对（太湖流域/太湖、五大连池/大连、九华山/华山…）
    #   · 意图解析能抽出景区名、不粘动词、不把"今天天气不错"当地名
